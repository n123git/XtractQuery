...
| Type | Description |
| - | - |
| 600 | Gets the absolute representation of a literal value or variable and sets to another variable. Non-numeric values coerce to `undefined`. <br>```$local1 = math_abs($local2);``` |
...
| 520 | If multiple arguments are passed it will return a random passed argument. Otherwise, instruction 520 gets a random value from 0 to a maximum defined by a literal value or variable and sets it to another variable.<br> The results for each data type are listed below:<br><ul><li>Integers: Returns a random integer in the range 0 to `math_abs(input)` - 1.</li><li>Floats: Returns a random float in the range 0.0 to `math_abs(input)`.</li><li>Undefined / no arguments: Returns a random float in the range 0.0 - 1.0.</li><li>Other: Returns `undefined`.</li></ul>```$local1 = random(5); // can return an int from 0-4```<br>```$local2 = random(5f); // can return a float from 0.0-5.0```<br>```$local2 = random(); // can return a float from 0.0-1.0```<br>```$local2 = random("hi"); // clears $local2 (sets it to undefined)```<br>```$local2 = random("apples", "oranges", "mangoes"); // returns either "apples", "oranges", or "mangoes"``` | <!-- no <br> needed; due to the list block-->
...
| 523 | Remaps the value of a variable to another literal value or variable, with a *required* default case.<br><pre>$local1 = $local2 switch <br>{<br>    1 => 99<br>    2 => $object1<br>    3 => $local3<br>    _ => 0<br>};</pre> |
